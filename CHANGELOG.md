...
### Changed
- `pq2_0` and `ptq1_0` at M 3 to 8 (M 4 to 8 on float16 activations), and
  `q4_0` at M 2 to 8, run on register-resident simdgroup-MMA verify
  kernels that decode each weight block straight into matrix fragments and
  read it once for all M rows. `KQ_VERIFY_MMA` forces or disables the
  route.
