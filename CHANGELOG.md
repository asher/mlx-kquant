...
- `sdpa_fa_verify` and `sdpa_decode_gqa_cascade` run faster when the folded
  ...
  draft tokens.
- On larger GPUs such as the M5 Max, `sdpa_fa_verify` and the shared prefix
  of `sdpa_decode_gqa_cascade` run faster below about 16k keys with few KV
  heads, and on 33 to 64 row folds past 16k keys.
...
### Changed
- On GPUs without NAX (M1 to M4), `pq2_0` and `ptq1_0` at M 3 to 8 and
  `q4_0` at M 2 to 8 run on register-resident simdgroup-MMA verify kernels
  that read each weight block once for all M rows. NAX GPUs keep their
  existing routes, and `KQ_VERIFY_MMA` forces or disables the new one.
- `q4_0`, `q4_1`, `q5_0` and `q5_1` join the split-K small-M tiles on GPUs
  without NAX, with the bm8 and bm16 variants, which speeds up their M 5 to
  32 band. NAX GPUs route these codecs as before.
...
- `arena_alloc` maps its buffer with mmap and unmaps it on release.
  ...
  decode-arena shrink or a model unload left tens of GB resident in the
  process and the kernel saw none of it as free.
...
- KVarN KV-cache quantization: `kvarn_quantize` / `kvarn_dequant`
  ...
  fused decode kernel that attends the records in place, plus
