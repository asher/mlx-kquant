...
def test_rhs_gather_sweep():
    ...
    for codec, (gtype, wpb, bpb, bits, is_kq) in CODECS.items():
        ...
        w = mx.array(wire)
        deq = mx.array(ref)  # [E, N, K] float32
        scales = mx.zeros((1,), dtype=mx.uint8)
        ne = wire.shape[0]
        ...
        for n_rows in (None, 96):
            ...
            dv = deq if n_rows is None else deq[:, :n_rows]
            for dtype in (mx.float16, mx.bfloat16):
                for B in (100, 256):
                    experts = np.sort(rng.integers(0, ne, size=B)).astype(
                        np.uint32
                    )
                    rhs = mx.array(experts)
                    x = mx.array(
                        (rng.standard_normal((B, 1, K)) * 0.1).astype(
                            np.float32
                        )
                    ).astype(dtype)
                    got = kq.gather_qmm(
                        x, wv, scales, codec, rhs_indices=rhs,
                        transpose=True, sorted_indices=True,
                    ).reshape(B, -1)
                    ctl = kq.gather_qmm(
                        x, wv, scales, codec, rhs_indices=rhs,
                        transpose=True, sorted_indices=False,
                    ).reshape(B, -1)
                    mx.eval(got, ctl)
                    g = np.array(got.astype(mx.float32))
                    c = np.array(ctl.astype(mx.float32))
                    rel = float(
                        np.linalg.norm(g - c) / (np.linalg.norm(c) + 1e-6)
                    )
