...
// weight nibbles are cached once per output row; the m-loop rebuilds the cheap
// per-position activation scaling and dots, so the dominant per-row weight read
// is amortized while the math stays bit-for-bit the qmv_fast path.
template <typename T, int group_size, int bits>
METAL_FUNC void kq_q4_0_verify_qmv_impl(
    const device uint8_t* w,
    const device T* x,
    device T* y,
    const constant int& in_vec_size,
...
#pragma unroll
        ...
        result[m][row] +=
            d * (acc[0] + acc[1] + acc[2] + acc[3] + sumy * U(-8));
      }
    }
  }
...
// threadgroup (vs 8 in the default), quadrupling the threadgroup count for
...
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_0_qmv_fast_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_0_qmv_fast_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}

template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_0_verify_qmv(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_0_verify_qmv_impl<T, group_size, bits>(
...
// Finer-tiled qmv (see kq_q4_0_qmv_fast_fine): 2 output rows per threadgroup.
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_0_qmv_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_0_qmv_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
...
// is amortized while the math stays bit-for-bit the qmv_fast path.
template <typename T, int group_size, int bits>
METAL_FUNC void kq_q4_1_verify_qmv_impl(
    const device uint8_t* w,
    const device T* x,
    device T* y,
    const constant int& in_vec_size,
...
// default variant (same per-row lane fold + simd_sum).
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_1_qmv_fast_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_1_qmv_fast_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}

template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_1_verify_qmv(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_1_verify_qmv_impl<T, group_size, bits>(
      w, x, y, in_vec_size, out_vec_size, vm, tid, simd_gid, simd_lid);
}
...
// Finer-tiled qmv (see kq_q4_1_qmv_fast_fine): 2 output rows per threadgroup.
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q4_1_qmv_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q4_1_qmv_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
...
#pragma unroll
        ...
        result[m][row] +=
            d * (acc[0] + acc[1] + acc[2] + acc[3] + sumy * U(-16));
      }
    }
  }
...
// default variant (same per-row lane fold + simd_sum).
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q5_0_qmv_fast_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q5_0_qmv_fast_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}

template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q5_0_verify_qmv(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q5_0_verify_qmv_impl<T, group_size, bits>(
      w, x, y, in_vec_size, out_vec_size, vm, tid, simd_gid, simd_lid);
}
...
// Finer-tiled qmv (see kq_q5_0_qmv_fast_fine): 2 output rows per threadgroup.
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_q5_0_qmv_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_q5_0_qmv_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
...
METAL_FUNC void kq_iq4_nl_qmv_impl(
  ...
  const int row_bytes = in_vec_size * KQ_IQ4_NL_BLOCK_BYTES / KQ_IQ4_NL_GROUP;
...
template <typename T, int group_size, int bits, int results_per_simdgroup = 4>
METAL_FUNC void kq_iq4_nl_qmv_fast_impl(
    const device uint8_t* w,
    const device T* x,
    device T* y,
    const constant int& in_vec_size,
    ...
    uint simd_gid,
    uint simd_lid) {
  kq_iq4_nl_qmv_impl<T, group_size, bits, results_per_simdgroup>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
...
struct KqIq4_nlBlockLoader {
  ...
  MLX_MTL_CONST short bytes_per_thread = n_reads / 2;
  MLX_MTL_CONST short half_block = weights_per_block / 2;
  static_assert(n_reads >= 2 && n_reads % 2 == 0, "IQ4_NL needs even n_reads.");
...
[[kernel]] void kq_iq4_nl_qmm_t(
  ...
  threadgroup T Ws[BN * BK_padded];
  using LoaderW =
      KqIq4_nlBlockLoader<T, BN, BK, BK_padded, 1, 2 * 2 * SIMD_SIZE>;
  kq_qmm_t_impl<T, LoaderW, aligned_N, BM, BK, BN>(
      w, x, y, Xs, Ws, K, N, M, K, tid, lid, simd_gid, simd_lid);
}
...
[[kernel]] void kq_iq4_nl_qmm_n(
  ...
  threadgroup T Ws[BK * BN_padded];
  using LoaderW =
      KqIq4_nlBlockLoader<T, BK, BN, BN_padded, 0, 2 * 2 * SIMD_SIZE>;
  kq_qmm_n_impl<T, LoaderW, BM, BK, BN>(
      w, x, y, Xs, Ws, K, N, M, tid, lid, simd_gid, simd_lid);
}

template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_iq4_nl_qmv_fast(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
  ...
  kq_iq4_nl_qmv_fast_impl<T, group_size, bits>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
...
// default variant (same per-row lane fold + simd_sum).
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_iq4_nl_qmv_fast_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_iq4_nl_qmv_fast_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}

template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_iq4_nl_qmv(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
  ...
  kq_iq4_nl_qmv_impl<T, group_size, bits>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}

// Finer-tiled qmv (see kq_iq4_nl_qmv_fast_fine): 2 output rows per threadgroup.
template <typename T, int group_size, int bits, bool batched>
[[kernel]] void kq_iq4_nl_qmv_fine(
    const device uint8_t* w,
    const device uint8_t* /* scales */,
    const device T* x,
    device T* y,
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  kq_iq4_nl_qmv_impl<T, group_size, bits, 1>(
      w, x, y, in_vec_size, out_vec_size, tid, simd_gid, simd_lid);
}
